...
XSH.aliases["hidefiles"] = (
    "defaults write com.apple.finder AppleShowAllFiles -bool false && killall Finder"
)
XSH.aliases["showfiles"] = (
...
def _pfd():
    ...
    result = subprocess.run(
        [
            "osascript",
            "-e",
            'tell application "Finder" to return POSIX path of (target of first window as text)',
        ],
        capture_output=True,
        text=True,
    )
...
def manp(args):
    ...
    pdf = subprocess.run(
        ["mandoc", "-T", "pdf", man.stdout.strip()], capture_output=True
    )
...
def mand(args):
    ...
    if not args:
        ...
        return 1
    result = subprocess.run(
        ["open", "-a", "Dash.app", f"dash://manpages%3A{args[-1]}"], capture_output=True
    )
